...
that employee's attendance records (when necessary). This applies to all employees the user is
...
in the Configuration menu. Navigate to the :menuselection:`Attendances application--> Configuration`
  ...
  - :guilabel:`Tolerance Time In Favor Of Employee`: enter the amount of time, in minutes, that an
    ...
    **not** penalized for their reduced hours.
    ...
    .. example::
       ...
       hours for the entire company is set from 9:00 AM to 5:00 PM.
...
.. important::
   In order for the *Attendances* app to log the location details, the user must allow their
   computer to access their location information.
...
To filter the results in the overview dashboard or to present different groups of information, click
the drop-down button in the :guilabel:`Search` bar above the dashboard, then select one of the
...
Odoo captures various time and location details when a user checks in and out :ref:`directly from
the database <attendances/check-in>`. To view the specific check in and/or check out details for an
employee, click on an individual entry in the overview dashboard.
...
- :guilabel:`Mode`: the mechanism used by the computer to pull the rest of the :guilabel:`Check In`
  and :guilabel:`Check Out` information from.
